...
#
# Scheduler - Aligned with Triton-distributed
#
################################################################################
import torch
...
def work_queue_list_to_device_tensor(sm_wq_list):
    ...
    # use int64 to handle large pointer values and -1 padding
    wq_tensor = torch.tensor(sm_wq_list, dtype=torch.int64, device=torch.cuda.current_device())
    num_tasks_tensor = torch.tensor(num_tasks_per_sm, dtype=torch.int32, device=torch.cuda.current_device())
    wq_tensor = wq_tensor.permute(1, 0, 2).reshape(wq_tensor.shape[1], wq_tensor.shape[0], wq_tensor.shape[2])
