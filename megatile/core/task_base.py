...
#
# Task Base - Aligned with Triton-distributed
#
################################################################################
...
class TaskDependency:
    ...
    def cover(self, other):
        return (other.layer_id == self.layer_id and other.task_id == self.task_id 
                and other.start_tiles >= self.start_tiles and other.end_tiles <= self.end_tiles)
...
class InputDependencyDesc:
    ...
    def __init__(self, input, require_full=True, start_indices=(), data_sizes=()):
        self.input = input
        self.require_full = require_full
        if require_full:
            self.start_indices = (0,) * len(input.shape)
            self.data_sizes = tuple(input.shape)
        else:
...
class TaskBase:
    ...
    def io_to_tuple(self):
        ...
        for tensor in all_tensors:
            ...
            assert len(tensor_tuple) % 2 == 0, "tensor data_ptr alignment"
            io_tuple += tensor_tuple
        return io_tuple
    
    def extra_params_to_tuple(self) -> Tuple[int]:
        # Subclasses should override this if they have extra_params
        return ()
    ...
    def encoding_with_deps(self, l, r) -> Tuple[int]:
        ...
        entrys += (l, r)
        assert len(entrys) % 2 == 0, "tensor data_ptr alignment"
